...
def write_scores(conf, result_file, stat, i_traj=None):
    ...
    sorted_ind = improvement.argsort(0)[::-1]
...
def combine_scores(conf, dir, only_first_n=None):
    files = glob.glob(dir + '/log/scores_*')
    ...
    scores = write_scores(conf, dir + '/results_all.txt', stat_array)
