...
from gcp.planning.cem_policy.utils.cost_fcn import EuclideanPathLength, LearnedCostEstimate
from gcp.planning.cem_policy.utils.sampler import FlatCEMSampler, SequentialHierarchicalSampler,\
                                                                         FlexibleHierarchicalSampler
...
class CEMPlanner:
    ...
    def _default_hparams(self):
        ...
        default_dict.update(ParamDict(
            verbose=False,
            dump_planning_data=False,
            log_cartgripper=False,
            max_logging_trajs=30,
            use_delta_state_actions=False,
            use_inferred_actions=True,
            max_seq_len=None,       # only used for model
        ))
        return default_dict
    ...
    def log_verbose(self, logger, step, phase, i_tr, dump_dir):
        if self._hp.verbose:
            if self._hp.log_cartgripper:
                logger.log_n_trajs_cartgripper_multiiter_multistep(self._logs, self._hp.max_logging_trajs,
                                                                   "elite_trajs_{}".format(i_tr), step, phase)
            else:
                logger.log_n_trajs_topdown_multiiter_multistep(self._logs, self._hp.max_logging_trajs,
                                                               "elite_trajs_{}".format(i_tr), step, phase)
    ...
    def _print_summary(log):
        def print_n(str, vals, format=False):
            for val in vals:
                str += "\t{:.2f},".format(val) if format else "\t{},".format(val)
            print(str)

        log = listdict2dictlist(log)
        log.dists = listdict2dictlist(log.dists)
        print("########################################")
        print_n("                ", ["It_{}".format(i) for i in range(len(log.elite_scores))])
        print_n("Mean Elite Score", map(lambda l: np.mean(l), log.elite_scores), format=True)
        print_n("Mean Dist Std   ", map(lambda l: np.mean(l), log.dists.std), format=True)
        print("########################################")
...
class HierarchicalCEMPlanner(CEMPlanner):
    ...
    def _default_hparams(self):
        ...
        # sampler params
        default_dict.update(ParamDict(
            sampler=SequentialHierarchicalSampler,
    ...
    def _get_best_rollouts(self, rollouts, goal_state, samples):
        if not isinstance(self._sampler, FlexibleHierarchicalSampler):
...
class ImageCEMPlanner(CEMPlanner):
    def log_verbose(self, logger, step, phase, i_tr, dump_dir):
        if i_tr >= 5:       # only log all details for initial iterations
            return
        if self._hp.dump_planning_data:
            os.makedirs(os.path.join(dump_dir, "planning"), exist_ok=True)
            with open(os.path.join(dump_dir, "planning/traj{}_raw_data.pkl".format(i_tr)), "wb") as F:
