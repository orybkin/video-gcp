from gcp.infra.datasets.save_util.record_saver import RecordSaver, float_feature, bytes_feature, int64_feature
import numpy as np
import os

from gcp.infra.agent.utils.hdf5_saver import HDF5Saver
...
def record_worker(queue, save_dir, sequence_length, seperate_good, traj_per_file, save_format, offset=0, split=(0.90, 0.05, 0.05)):
    print('started saver with PID:', os.getpid())
    print('saving to {}'.format(save_dir))
    if save_format == 'tfrec':
        saver = GeneralAgentSaver(save_dir, sequence_length, seperate_good, traj_per_file, offset, split)
    elif save_format == 'hdf5':
        saver = HDF5Saver(save_dir, traj_per_file, offset, split)
    else:
