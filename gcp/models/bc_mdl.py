...
from blox.torch.subnetworks import Predictor, RecurrentPolicyModule
...
class BehavioralCloningModel(BaseModel):
    ...
    def forward(self, inputs, phase='train'):
        ...
        if not 'enc_demo_seq' in inputs:
            enc_demo_seq, _ = self.encoder(inputs.demo_seq[:, 0]) if self._hp.train_first_action_only \
                                    else batch_apply(inputs.demo_seq, self.encoder)
            if self._hp.train_first_action_only: enc_demo_seq = enc_demo_seq[:, None]
            enc_demo_seq = enc_demo_seq.detach() if self.detach_enc else enc_demo_seq

        enc_goal, _ = self.encoder(inputs.I_g)
        n_dim = len(enc_goal.shape)
        fused_enc = torch.cat((enc_demo_seq, enc_goal[:, None].repeat(1, enc_demo_seq.shape[1], *([1]*(n_dim-1)))), dim=2)
        #fused_enc = torch.cat((enc_demo_seq, enc_goal[:, None].repeat(1, enc_demo_seq.shape[1], 1, 1, 1)), dim=2)

        if self._hp.reactive:
            actions_pred = batch_apply(fused_enc, self.policy)
        else:
