...
class CostModel(BaseModel):
    ...
    def loss(self, inputs, model_output, add_total=True):
        return AttrDict(
            cost_estimation=L2Loss(1.0)(model_output.cost, model_output.cost_target)
        )
    ...
    def _fast_path_dist_cost(self, inputs):
        ...
        cum_diff = torch.cumsum(torch.norm(inputs.demo_seq[:, 1:] - inputs.demo_seq[:, :-1], dim=-1), dim=1)
    ...
    def _general_cost(self, inputs):
        ...
        for b in range(batch_size):
            ...
            gt_cost.append(self._gt_cost_fcn([inputs.demo_seq[b, start_idx:end_idx+1].data.cpu().numpy()],
                                             inputs.demo_seq[b, end_idx].data.cpu().numpy()))
