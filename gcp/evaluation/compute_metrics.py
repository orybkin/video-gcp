...
from blox.tensor.ops import get_dim_inds, batchwise_index
from blox.torch.evaluation import ssim, psnr, mse
from blox.utils import timed
from gcp.rec_planner_utils.vis_utils import plot_pruned_tree, make_gif
...
class Evaluator:
    ...
    def eval_single(self, inputs, model_output, sample_n=0):
        ...
        store_states = "demo_seq_states" in inputs and (inputs.demo_seq_states.shape[-1] == 2 or
                                                        inputs.demo_seq_states.shape[-1] == 5)
        # TODO paralellize DTW
        for b in range(bsize):
            input_seq = input_images[b, :inputs.end_ind[b]+1]
            input_len = input_seq.shape[0]
            gen_seq, matching_output = self._dense_rec_module.get_sample_with_len(b, input_len, model_output, inputs, self._pruning_scheme)
            input_seq, gen_seq = input_seq[1:-1], gen_seq[1:-1]     # crop first and last frame for eval (conditioning frames)
            state_seq = inputs.demo_seq_states[b, :input_len] if store_states else None

            full_gen_seq, gen_seq = self.compute_metrics(b, gen_seq, input_seq, model_output, sample_n)
            ...
            if sample_n < self.N_PLOTTED_ELEMENTS:
                pred_len = model_output.end_ind[b].data.cpu().numpy() + 1 if 'end_ind' in model_output else input_len
                pred_len_seq, _ = self._dense_rec_module.get_sample_with_len(b, pred_len, model_output, inputs,
    ...
    def dump_trees(self, it):
        ...
        with self._logger.log_to('trees', it, 'image'):
            tree_imgs = plot_pruned_tree(img_dict.model_output.tree, no_pruning, plot_matched).detach().cpu().numpy()
        ...
        with self._logger.log_to('matchings', it, 'image'):
            try:
                for i in range(min(Evaluator.N_PLOTTED_ELEMENTS, self.full_evaluation.gen_images.shape[0])):
                    im = self._dense_rec_module.eval_matcher.vis_matching(self.full_evaluation.gen_images[i].matching_outputs)
                    self._logger.log(im)
