...
from gcp.models.trees.attentive import AttentiveMatcherWithIntegration
...
def plot_balanced_tree_with_actions(model_output, inputs, n_logged_samples, get_prob_fcn=None):
    ...
    if inputs is not None:
        im[:, :res, :res] = imgtensor2np(inputs.demo_seq[:n_logged_samples, 0], n_logged_samples).transpose(0, 2, 3, 1)
        im[:, :res, -res:] = imgtensor2np(batchwise_index(inputs.demo_seq[:n_logged_samples], model_output.end_ind[:n_logged_samples]),
...
def plot_gt_matching_overview(model_output, inputs, plot_attr='match_dist'):
    ...
    if len(inputs.demo_seq_images.shape) > 2:
        assert inputs.demo_seq_images.shape[3] == inputs.demo_seq_images.shape[4]     # code can only handle square images atm
    batch, n_gt, channels, res, _ = inputs.demo_seq_images.shape
    n_logged_samples = PARAMS.n_logged_samples
    ...
    im[:, :res*n_gt, :res] = get_strip(inputs.demo_seq_images, gt=True)
...
def plot_gt_action_matching_overview(model_output, inputs, hp, n_logged_samples=3):
    """Plots overview of which predicted frames contributed to which subgoals."""
    assert inputs.demo_seq.shape[3] == inputs.demo_seq.shape[4]  # code can only handle square images atm
    assert inputs.actions.shape[-1] == 2     # code can only handle 2-dim actions
    batch, n_gt_imgs, channels, res, _ = inputs.demo_seq.shape
    _, n_gt_actions, action_dim = inputs.actions.shape
    tree = model_output.tree
    max_depth = tree.depth
    n_sg = (2**max_depth) - 1

    im_height = (n_gt_imgs+max_depth*2+1) * res     # plot all actions (*2) and start/end frame again (+1)
    im_width = (n_sg + 6) * res     # +1 for the non-symmetric action in the middle
    im = np.asarray(0.7 * np.ones((n_logged_samples, im_height, im_width, 3)), dtype=np.float32)

    # insert ground truth images on the left, soft estimates on the right, top to bottom
    # insert raw subgoal predictions tree at the bottom, left to right in depth-first order
    im[:, :res*n_gt_imgs, :res] = imgtensor2np(inputs.demo_seq, n_logged_samples).transpose(0, 1, 3, 4, 2)\
                                                                .reshape(n_logged_samples, res*n_gt_imgs, res, channels)
    im[:, :res * n_gt_imgs, -res:] = imgtensor2np(model_output.soft_matched_estimates, n_logged_samples).transpose(0, 1, 3, 4, 2) \
                                                                .reshape(n_logged_samples, res * n_gt_imgs, res, channels)
    im[:, :res * n_gt_actions, res:2*res] = batch_action2img(tensor2np(inputs.actions, n_logged_samples), res, channels) \
                                                                .reshape(n_logged_samples, res * n_gt_actions, res, channels)
    im[:, :res * n_gt_actions, -2 * res:-res] = batch_action2img(tensor2np(model_output.soft_action_estimates, n_logged_samples), res, channels) \
                                                                .reshape(n_logged_samples, res * n_gt_actions, res, channels)
    im[:, -(2*max_depth+1)*res:, 2*res:-2*res] = plot_balanced_tree_with_actions(model_output, inputs, n_logged_samples)

    # sort action matching distributions in plotted depth-first order
    match_dists = sort_actions_depth_first(model_output, ['a_l_match_dist', 'a_r_match_dist'], n_logged_samples)
    # match_dists = torch.zeros((n_logged_samples, 2*n_sg+1, n_gt_actions), device=model_output.norm_gt_action_match_dists.device)
    # for i, segment in enumerate(tree):
    #     match_dists[:, 2 * (i+1) - 2 ** (segment.depth - 1) - 1] = segment.subgoal.a_l_match_dist[:n_logged_samples]
    #     match_dists[:, 2 * (i+1) + 2 ** (segment.depth - 1) - 1] = segment.subgoal.a_r_match_dist[:n_logged_samples]
    gt_match_dists = AttentiveMatcherWithIntegration(hp, None, None).get_w(match_dists, inputs.pad_mask[:n_logged_samples])  # feed dummy for actions
    match_dists = tensor2np(gt_match_dists, n_logged_samples)

    fig_height, fig_width = res, (n_sg+0.5) * res
    dpi = 10
    for i in range(n_gt_actions):
        for b in range(n_logged_samples):
            match_plot = plot_dists([match_dists[b, :, i]], fig_height, fig_width, dpi, linewidth=3.0)
            im[b, i*res : (i+1)*res, int(2.5*res):int(2.5*res + fig_width)] = match_plot
...
def plot_pruned_seqs(model_output, inputs, n_logged_samples=3, max_seq_len=None):
    ...
    if inputs is not None:
        im[:, :res, :(n_gt_imgs*res)] = imgtensor2np(inputs.demo_seq, n_logged_samples).transpose(0, 3, 1, 4, 2)\
                                            .reshape(n_logged_samples, res, n_gt_imgs*res, channels)
...
def plot_demo_following_overview(demo_seq, exec_seq, goal):
    """Plots overview of pruned subplans.
    :arg demo_seq: sequence of demo frames, [N, H, W, C]
    ...
    demo_len = demo_seq.shape[0]
    im[res: res*2, :demo_len*res] = demo_seq.transpose(1, 0, 2, 3).reshape(res, demo_len* res, channels)
...
def plot_inverse_model_actions(model_output, inputs, n_logged_samples=5):
    ...
    if len(model_output.action_targets.shape) <= 2:
        ...
        model_output.img_t0, model_output.img_t1 = inputs.demo_seq[:, 0], inputs.demo_seq[:, 1]
