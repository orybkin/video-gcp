...
def get_default_gcp_hyperparameters():
    ...
    default_dict.update({
        ...
        'sg_exist_weight': 1.,
        'sg_action_exist_weight': 1.,
        'supervise_attn_weight': 0.,
        'free_nats': 0,
    })
    ...
    default_dict.update({
        ...
        'leaf_nodes_only': False,  # if True, uses only leaf nodes for matching
        
        'timestep_cond_attention': False,  # if True, predicts fraction first, then attention
        'one_hot_attn_time_cond': False,  # if True, conditions attention on one-hot time step index
        'forced_attention': False,  # if True, forces attention to single matching frame
        ...
        'lstm_warmup_cycles': 0,  # number of warmup iterations for LSTM
        ...
        'supervise_fraction_weight': 0.0,
        'supervise_fraction': 'top_index',
        ...
        'equal_weight_layer': False,
        'untied_layers': False,
        'supervised_decoder': False,
        'states_inference': False,
    })
    
    # Outdated RecPlan params
    default_dict.update({
        'regress_exist_prob': False,  # whether or not to regress existence probability
        'regress_action_exist_prob': False,  # whether or not to regress existence probability for actions
        'dense_rec_type': 'none',  # ['none', 'discrete', 'softmax', 'linear', 'node_prob', action_prob].
        'timestep_decision': 'default',  # ['default', 'half']. Always 'default'.
        'one_step_planner': 'discrete',  # ['discrete', 'continuous', 'sh_pred']. Always 'sh_pred' for HEDGE models
        'detach_existence_pred': True,  # if True, stops gradients from existence estimation into the model
        'detach_action_pred': True,  # if True, stops gradients from action prediction into the model
        'mask_inf_attention': False,  # if True, masks out inference attention outside the current subsegment
        'exist_thresh': 0.5,  # minimal existence prob for a node not to be pruned in test tree
        'parent_exist_thresh': 0.3,  # minimal existence prob for a node not to be pruned in test tree
        'matcher': 'frames',  # Matching loss form ['loss', 'frames', 'exp', 'lf']. Always 'loss'.
        'minent': 1e-6,
    })
    
    # Matching params
    default_dict.update(AttrDict(
        norm_match_dists=True,  # if True, normalize the node matching distributions per groundtruth frame
        use_moment_matching=False,  # if True, fits Gaussian to attention matching distribution
    ...
    default_dict.update(AttrDict(
        ...
        log_maze_topdown=False,  # if True, logs topdown view of predicted state trajectories in maze
        log_states_2d=False,  # if True, logs 2D plot of first two state dimensions
        log_sawyer=False,  # if True, logs sawyer from states
