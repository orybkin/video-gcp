...
import torch
import torchvision
from gcp.prediction.utils.visualization import \
    plot_hierarchical_match_dists, plot_gt_matching_overview, plot_val_tree, plot_pruned_seqs, plot_balanced_tree, \
    plot_planning_overview, plot_pruned_tree, make_gif, plot_graph, param, \
    plot_traj_following_overview, fig2img, plot_inverse_model_actions
...
class HierarchyLogger(Logger):
    ...
    def log_traj_following_overview(self, traj_seq=None, exec_seq=None, goal=None, _name=None, _step=None, phase=None):
        im = plot_traj_following_overview(traj_seq, exec_seq, goal)
        self._log_plot_img(im, _name, _step, phase)
    ...
    def log_topdown_traj_overview(self, inputs, name, step, phase):
        ...
        im = np.stack([self.dummy_env.render_top_down(inputs.traj_seq_states[i, :end_ind].data.cpu().numpy()) for i, end_ind in
